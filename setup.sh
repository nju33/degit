...
replace () {
  ...
  do
    file_name=$1
    file_name_in_constant="$(F="$file_name" node -p "process.env.F.replace(/([a-zA-Z])/g, (_, p) => p.toUpperCase()).replace(/-/g, () => '_'))")"
    ...
    if command -v gsed; then
      gsed -i "s/DUCK_NAME_IN_CONSTANT/$file_name_in_constant/g" "$f"
      ...
      sed -i "s/DUCK_NAME_IN_CONSTANT/$file_name_in_constant/g" "$f"
